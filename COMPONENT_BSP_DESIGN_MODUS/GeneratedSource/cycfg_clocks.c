...
#include "cycfg_clocks.h"
	...
	const cyhal_resource_inst_t CLK_PD_ISNK_obj = 
	{
		.type = CYHAL_RSC_CLOCK,
		.block_num = CLK_PD_ISNK_HW,
		.channel_num = CLK_PD_ISNK_NUM,
	};
...
#endif //defined (CY_USING_HAL)
#if defined (CY_USING_HAL)
	const cyhal_resource_inst_t CLK_REFGEN_obj = 
	{
		.type = CYHAL_RSC_CLOCK,
		.block_num = CLK_REFGEN_HW,
		.channel_num = CLK_REFGEN_NUM,
	};
#endif //defined (CY_USING_HAL)
...
void init_cycfg_clocks(void)
{
	...
	Cy_SysClk_PeriphSetDivider(CY_SYSCLK_DIV_16_BIT, 0U, 479U);
	Cy_SysClk_PeriphEnableDivider(CY_SYSCLK_DIV_16_BIT, 0U);
#if defined (CY_USING_HAL)
	cyhal_hwmgr_reserve(&CLK_PD_ISNK_obj);
#endif //defined (CY_USING_HAL)
...
#if defined (CY_USING_HAL)
	cyhal_hwmgr_reserve(&CLK_REFGEN_obj);
#endif //defined (CY_USING_HAL)
}
